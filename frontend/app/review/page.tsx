...
export default function ReviewPage() {
  ...
  const handleStartSpeechRecognition = () => {
    ...
    recognition.onresult = (event: any) => {
      ...
      setFeedback('Speech captured! Tap "Score" to evaluate.');
    };
  ...
  if (loading) {
    return (
      <div className="min-h-screen-safe bg-premium-surface flex items-center justify-center">
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen-safe bg-premium-surface flex items-center justify-center px-4">
  ...
  return (
    <div className="min-h-screen-safe bg-premium-surface flex flex-col">
      {/* Top Bar */}
      <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-border/50 safe-top">
        <div className="max-w-2xl mx-auto px-3 sm:px-6 py-2.5 sm:py-3 flex items-center justify-between gap-2">
          <button
            onClick={() => router.push('/dashboard')}
            className="flex items-center gap-1 sm:gap-1.5 text-xs sm:text-sm text-muted-foreground hover:text-foreground active:text-foreground transition-colors touch-target flex-shrink-0"
          >
          ...
          {/* Mode Selector */}
          <select
            className="text-xs bg-muted/30 border border-border/50 rounded-lg px-2 sm:px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-ring/20 transition-all max-w-[110px] sm:max-w-none touch-target"
            value={mode}
      ...
      {/* Review Content - flex-1 to fill available space */}
      <main className="flex-1 flex flex-col max-w-2xl w-full mx-auto px-3 sm:px-6 py-5 sm:py-8 md:py-12">
        <Card className="overflow-hidden flex-1 flex flex-col">
          <CardContent className="p-4 sm:p-6 md:p-10 flex-1 flex flex-col justify-center">
            <div className="text-center">
              {/* Item Type Badge */}
              <div className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-full bg-muted/50 mb-4 sm:mb-6">
                {modeIcons[mode]}
                <span className="text-[11px] sm:text-xs font-medium text-muted-foreground capitalize">{currentItem.type}</span>
              </div>

              {/* Flashcard Mode */}
              {mode === 'flashcard' && (
                <div className="animate-fade-in">
                  <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4 break-words tracking-tight">{currentItem.content}</h2>
                  {loadingTranslation ? (
                    ...
                    <p className="text-lg sm:text-xl md:text-2xl text-muted-foreground break-words">{flashcardPair.shown}</p>
                  ) : null}
                </div>
              )}
              ...
              {mode === 'cloze' && (
                <div className="animate-fade-in mb-4 sm:mb-6">
                  <p className="text-lg sm:text-xl md:text-2xl font-semibold break-words mb-6 sm:mb-8">{clozeData.prompt}</p>
                  <input
              ...
              {mode === 'listening' && (
                <div className="animate-fade-in mb-4 sm:mb-6">
                  <p className="text-sm text-muted-foreground mb-4 sm:mb-6">Listen carefully and type what you hear</p>
                  <Button onClick={handlePlayAudio} variant="outline" className="mb-4 sm:mb-6 touch-target">
              ...
              {mode === 'writing' && (
                <div className="animate-fade-in mb-4 sm:mb-6">
                  ...
                  <p className="text-lg sm:text-xl font-semibold mb-4 sm:mb-6">{currentItem.content}</p>
                  <textarea
                    className="w-full bg-muted/30 border border-border/50 rounded-xl px-3 sm:px-4 py-3 text-base min-h-[100px] sm:min-h-[120px] resize-none focus:outline-none focus:ring-2 focus:ring-ring/20 focus:border-foreground/20 transition-all duration-200"
              ...
              {mode === 'speaking' && (
                <div className="animate-fade-in mb-4 sm:mb-6">
                  <p className="text-sm text-muted-foreground mb-3 sm:mb-4">Read aloud:</p>
                  <p className="text-lg sm:text-xl md:text-2xl font-semibold mb-4 sm:mb-6">{currentItem.content}</p>
                  {recognizedText && (
                    <div className="mb-4 p-3 sm:p-4 bg-muted/30 rounded-xl text-left">
                  ...
                  <div className="flex flex-col items-center gap-3">
                    <div className="flex justify-center gap-2 sm:gap-3 flex-wrap">
                      {!isListening ? (
                        <Button onClick={handleStartSpeechRecognition} variant="outline" className="touch-target">
                        ...
                        <Button variant="destructive" onClick={handleStopSpeechRecognition} className="touch-target">
                          <div className="w-3 h-3 rounded-full bg-white animate-pulse mr-1.5 sm:mr-2" />
                      ...
                      >
                        <span className="text-sm">{checking ? 'Scoring...' : 'Score'}</span>
                      </Button>
                    </div>
              ...
              {mode === 'flashcard' && flashcardPair?.shown && !loadingTranslation && !showFlashcardCorrection && (
                <div className="flex gap-3 sm:gap-4 justify-center mt-8 sm:mt-10 px-2">
                  <Button
                    size="lg"
                    variant="outline"
                    className="flex-1 sm:flex-none sm:min-w-[120px] max-w-[160px] border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700 active:bg-red-100 h-12 touch-target"
                    onClick={() => handleFlashcardAnswer(false)}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-1.5 sm:mr-2 flex-shrink-0">
                  ...
                  <Button
                    size="lg"
                    className="flex-1 sm:flex-none sm:min-w-[120px] max-w-[160px] bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 h-12 touch-target"
              ...
              {mode === 'flashcard' && showFlashcardCorrection && flashcardPair?.correct && (
                <div className="mt-6 sm:mt-8 p-4 sm:p-5 bg-amber-50 border border-amber-100 rounded-xl animate-fade-in">
                  <p className="text-sm text-amber-800">
                    Correct translation: <span className="font-semibold">{flashcardPair.correct}</span>
                  </p>
                  <Button onClick={handleContinueAfterCorrection} className="mt-4 touch-target" size="sm">
                    Continue
                  </Button>
                </div>
              )}
              ...
              {mode === 'cloze' && (
                <div className="mt-4 sm:mt-6">
                  <Button onClick={() => handleCheckText(clozeData.answer)} className="w-full sm:w-auto touch-target">Check Answer</Button>
                </div>
              )}
              {mode === 'listening' && (
                <div className="mt-4 sm:mt-6">
                  <Button onClick={() => handleCheckText(currentItem.content)} className="w-full sm:w-auto touch-target">Check Answer</Button>
                </div>
              )}

              {/* Writing Actions */}
              {mode === 'writing' && (
                <div className="mt-4 sm:mt-6 flex flex-col sm:flex-row justify-center gap-2.5 sm:gap-3">
                  <Button onClick={handleWritingCheck} disabled={checking} className="touch-target">
                    {checking ? (
                  ...
                  <Button variant="outline" onClick={handleSaveWritingResult} disabled={aiScore === null} className="touch-target">
                    Save Result
                  </Button>
                </div>
              )}
              ...
              {mode === 'speaking' && (
                <div className="mt-4 sm:mt-6">
                  <Button variant="outline" onClick={() => handleAnswer((aiScore ?? 0) >= 70 ? 'correct' : 'incorrect')} disabled={aiScore === null} className="touch-target">
                    Save Result
                  </Button>
                </div>
              )}
              ...
              {/* Feedback */}
              {feedback && (
                <p className="mt-4 sm:mt-5 text-sm text-muted-foreground animate-fade-in">{feedback}</p>
              )}

              {/* AI Results Panel */}
              {(aiScore !== null || aiCorrections.length > 0 || aiTranscript) && (
                <div className="mt-4 sm:mt-6 p-3.5 sm:p-5 bg-muted/30 rounded-xl text-left text-sm space-y-2.5 sm:space-y-3 animate-fade-in">
                  {aiScore !== null && (
                    <div className="flex items-center gap-2">
                      <span className="text-[11px] sm:text-xs font-medium text-muted-foreground uppercase tracking-wider">Score</span>
                      <span className={`text-base sm:text-lg font-bold ${aiScore >= 70 ? 'text-emerald-600' : 'text-red-500'}`}>
                        {aiScore}
                      </span>
                    </div>
                  )}
                  {aiTranscript && (
                    <div>
                      <span className="text-[11px] sm:text-xs font-medium text-muted-foreground uppercase tracking-wider">Transcript</span>
                      <p className="mt-1 text-sm">{aiTranscript}</p>
                    </div>
                  )}
                  {aiPronunciationIssues.length > 0 && (
                    <div>
                      <span className="text-[11px] sm:text-xs font-medium text-muted-foreground uppercase tracking-wider">Pronunciation Issues</span>
                  ...
                  {aiCorrections.length > 0 && (
                    <div>
                      <span className="text-[11px] sm:text-xs font-medium text-muted-foreground uppercase tracking-wider">Corrections</span>
                      <ul className="mt-1 space-y-1.5">
                        {aiCorrections.map((c, idx) => (
                          <li key={idx} className="text-sm flex flex-wrap items-baseline gap-x-1.5">
